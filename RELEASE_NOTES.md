...
- Added a SQLite SQL import profile with 8 entity tables, 9 association tables, graph-oriented indexes, `process_edges`, and `property_value_orphans`.
...
- Aligned current code and tests around the core vocabulary `LabProcess`, `LabProtocol`, `inputs`, `outputs`, and `executesProtocol`.

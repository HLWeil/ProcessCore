...
DROP VIEW IF EXISTS property_value_orphans;
DROP VIEW IF EXISTS process_edges;

DROP TABLE IF EXISTS data_additional_property;
DROP TABLE IF EXISTS material_additional_property;
...
CREATE TABLE lab_process (
  ...
  additional_type TEXT,
  name TEXT NOT NULL,
  executes_protocol_id TEXT REFERENCES lab_protocol(id) ON DELETE RESTRICT
);
...
CREATE TABLE dataset_process (
  dataset_id TEXT NOT NULL REFERENCES dataset(id) ON DELETE CASCADE,
  position INTEGER NOT NULL CHECK (position >= 0),
  process_id TEXT NOT NULL REFERENCES lab_process(id) ON DELETE RESTRICT,
  PRIMARY KEY (dataset_id, position)
);
...
CREATE TABLE dataset_additional_property (
  dataset_id TEXT NOT NULL REFERENCES dataset(id) ON DELETE CASCADE,
  position INTEGER NOT NULL CHECK (position >= 0),
  property_value_id TEXT NOT NULL REFERENCES property_value(id) ON DELETE RESTRICT,
  PRIMARY KEY (dataset_id, position)
);

CREATE TABLE protocol_parameter (
  protocol_id TEXT NOT NULL REFERENCES lab_protocol(id) ON DELETE CASCADE,
  position INTEGER NOT NULL CHECK (position >= 0),
...
CREATE TABLE process_io (
  process_id TEXT NOT NULL REFERENCES lab_process(id) ON DELETE CASCADE,
  direction TEXT NOT NULL CHECK (direction IN ('input', 'output')),
  position INTEGER NOT NULL CHECK (position >= 0),
  material_id TEXT REFERENCES material(id) ON DELETE RESTRICT,
...
CREATE TABLE material_additional_property (
...
CREATE INDEX idx_lab_process_protocol
  ON lab_process(executes_protocol_id);

CREATE INDEX idx_lab_protocol_intended_use
  ON lab_protocol(intended_use_id);

CREATE INDEX idx_process_parameter_value_property
  ON process_parameter_value(property_value_id);

CREATE INDEX idx_property_value_instance_of
  ON property_value(instance_of_id);

CREATE INDEX idx_property_value_name_value
  ON property_value(name_tan, value);

CREATE INDEX idx_dataset_additional_property_value
  ON dataset_additional_property(property_value_id);

CREATE INDEX idx_protocol_additional_property_value
  ON protocol_additional_property(property_value_id);

CREATE INDEX idx_material_additional_property_value
  ON material_additional_property(property_value_id);

CREATE INDEX idx_data_additional_property_value
  ON data_additional_property(property_value_id);
...
SELECT
  ...
  CASE WHEN consumed.material_id IS NOT NULL THEN 'material' ELSE 'data' END AS input_kind,
  coalesce(consumed.material_id, consumed.data_id) AS input_id,
  CASE WHEN produced.material_id IS NOT NULL THEN 'material' ELSE 'data' END AS output_kind,
  coalesce(produced.material_id, produced.data_id) AS output_id
...
CREATE VIEW property_value_orphans AS
SELECT pv.id
FROM property_value AS pv
WHERE NOT EXISTS (
  SELECT 1 FROM dataset_additional_property AS a WHERE a.property_value_id = pv.id
)
AND NOT EXISTS (
  SELECT 1 FROM protocol_additional_property AS a WHERE a.property_value_id = pv.id
)
AND NOT EXISTS (
  SELECT 1 FROM material_additional_property AS a WHERE a.property_value_id = pv.id
)
AND NOT EXISTS (
  SELECT 1 FROM data_additional_property AS a WHERE a.property_value_id = pv.id
)
AND NOT EXISTS (
  SELECT 1 FROM process_parameter_value AS a WHERE a.property_value_id = pv.id
);

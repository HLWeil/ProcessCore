...
- Add a second, more schema-native representation where the same semantics are expressed as a `Annotation` decoration attached to `Data` via `additionalProperty`.
- Treat the `Annotation`-based form as the flexible extension path, and describe the core `DataContext` form as a convenience model that can be mapped into it.
- Generalize `additionalProperty` as the standard extension hook for all ProcessCore entity types, not just `Dataset` and `Sample`.
...
- Add explicit mapping notes showing which fields correspond to RO-Crate fragment concepts such as `about`, `subjectOf`, `usageInfo`, `pattern`, and fragment description `Annotation`s.

### 2. Add a datamap decoration based on `Annotation`
...
- Add a decoration overview that explains the alternate representation:
  ...
  - the attached object is a specialized `Annotation` representing a data context / fragment descriptor.
- Add a stub for the datamap-specific `Annotation` subtype.
- Document how this decoration maps to the current datamap reference:
  - reference profile: fragment description is a `Annotation`,
  - ARC core variant: attach that `Annotation` directly to `Data`,
...
- Update [spec/core/README.md](spec/core/README.md) to:
  ...
  - show that `additionalProperty` is no longer limited to `Sample` and `Dataset`.
...
- Naming: whether the `Annotation` subtype should be called `DataContext`, `DataContextValue`, or `FragmentDescriptor`.

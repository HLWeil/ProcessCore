...
module Expect = 
    ...
    let compositeColumnsEqual (t1 : CompositeColumn) (t2 : CompositeColumn) (message : string) = 
        Expect.equal t1.Header t2.Header $"{message}. Headers do not match"
        sequenceEqual t1.Cells t2.Cells $"{message}. Cells do not match"


    let arcTableEqual (t1 : Table) (t2 : Table) (message : string) = 
        sequenceEqual t1.Columns t2.Columns $"{message}. Columns do not match"

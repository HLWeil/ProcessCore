...
let tests = testList "Fragment selectors" [
    ...
    testList "Traversal" [
        ...
        testCase "whole file reaches sample through contained fragment" <| fun _ ->
            ...
            p.SetInputSample(source)
            p.SetOutputData(fragment)
            let ds = datasetWithFakeProvider ()
            ds.AddProcess(p)
        ...
        testCase "connect only through correct fragment containment (separate processes)" <| fun _ ->
            ...
            p1.SetOutputData(fragment1)
            let p2 = Process("produce-2")
            p2.SetInputSample(source2)
            p2.SetOutputData(fragment2)
            let ds = datasetWithFakeProvider ()
            ds.AddProcess(p1)
            ds.AddProcess(p2)
        ...
        testCase "sample reaches final data through fragment of full file" <| fun _ ->
            ...
            p1.SetInputSample(source)
            p1.SetOutputData(intermediaryFile)
            p2.SetInputData(intermediaryFragment)
        ...
        testCase "sample reaches final data through fragment of fragment" <| fun _ ->
            ...
            let p2 = Process("produce-output")
            p1.SetInputSample(source)
            p1.SetOutputData(intermediaryOuterFragment)
            p2.SetInputData(intermediaryFragmentContained)
            p2.SetOutputData(outputData1)
            let ds = datasetWithFakeProvider ()
            ds.AddProcess(p1)
            ds.AddProcess(p2)
        ...
        testCase "sample reaches final data through fragment of fragment ignore disjunct" <| fun _ ->
            ...
            p2.SetInputData(intermediaryFragmentContained)
            p2.SetOutputData(outputData1)
            let p3 = Process("consume-fragment")
            p3.SetInputData(intermediaryFragmentNotContained)
            p3.SetOutputData(outputData2)
            let ds = datasetWithFakeProvider ()
            ds.AddProcess(p1)
            ds.AddProcess(p2)
            ds.AddProcess(p3)

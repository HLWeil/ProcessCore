...
let tests = testList "PathQueries" [
    ...
    testCase "Path.Length" <| fun _ ->
        let f    = makeFixtureA()
        let path = Path(ResizeArray<LabProcess>([| f.P1; f.P2; f.P3 |]))
    ...
    testCase "empty Path" <| fun _ ->
        let path = Path(ResizeArray<LabProcess>())
    ...
    testCase "Path.Materials()" <| fun _ ->
        let f    = makeFixtureA()
        let path = Path(ResizeArray<LabProcess>([| f.P1; f.P2; f.P3 |]))
        let names = path.Materials() |> Seq.map (fun m -> m.Name) |> Set.ofSeq
        Expect.equal names (Set.ofList ["Source1";"Sample1";"Sample2"])
            "Materials excludes rawData1.csv"
    ...
    testCase "Path.ContainsNode — present" <| fun _ ->
        let f    = makeFixtureA()
        let path = Path(ResizeArray<LabProcess>([| f.P1; f.P2; f.P3 |]))
        Expect.isTrue (path.ContainsNode(MaterialNode f.Sample1))
            "Sample1 is in the path"

    testCase "Path.ContainsNode — absent" <| fun _ ->
        let f    = makeFixtureA()
        let path = Path(ResizeArray<LabProcess>([| f.P1 |]))
    ...
    // ── Path.AllPropertyValues / PropertyValuesByName ─────────────────────────

    testCase "Path.AllPropertyValues — all 4 sources" <| fun _ ->
    ...
    testCase "Path.PropertyValuesByName" <| fun _ ->
        let f    = makeFixtureA()
        let path = Path(ResizeArray<LabProcess>([| f.P1; f.P2; f.P3 |]))
        let pvs  = path.PropertyValuesByName("temperature")

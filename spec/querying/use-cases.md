...
In addition to the core entities defined in the core specification and their properties, we also want a new additional type: **Path**

This type contains a collection of processes connected through their shared nodes (input and output entities). 
...
- Dataset: Give me all samples which result from specific growth temperature?
    ...
    - Technical formulation: Give me all samples where in a path leading up to this sample, a propertyValue in process with given protocolType equals a specific value.
    - Requirements: 

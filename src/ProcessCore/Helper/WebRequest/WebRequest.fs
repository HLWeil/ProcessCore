...
let downloadFile url =
    ...
    if ProcessCore.WebRequestHelpers.NodeJs.isNode() then
        ...
        ProcessCore.WebRequestHelpers.NodeJs.downloadFile url
    else
        browserAndDotnet()
    #endif

    #if FABLE_COMPILER_PYTHON
    ProcessCore.WebRequestHelpers.Py.downloadFile url
    #endif
